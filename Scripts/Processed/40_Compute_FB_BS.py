...
###############################################################################################
# CODE DESCRIPTION
# 40_Compute_FB_BS.py computes the values of the Area Under the ROC curve with the trapezoidal 
...
for ind in range(len(Prob_Thr_list)+1):
      ...
      for indStepF in range(len(StepF_list)):
            ...
            for ind_repBS in range(RepetitionsBS+1):
                  ...
                  # Computing AROCt
                  AROCt_array[indStepF, ind_repBS+1] =  np.nansum(Count_EM) / np.nansum(Count_OBS)
      ...
      FileNameOUT_temp = "FB_" + f"{Acc:02d}" + "h_" + SystemFC + "_" + str(Perc_VRT) + "_" + Name
