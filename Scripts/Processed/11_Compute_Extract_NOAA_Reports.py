...
######################################################################################################################################################################
# CODE DESCRIPTION
# 05_Compute_Extract_NOAA_Reports.py extracts the flash flood reports from the NOAA database,
...
DirOUT = "Data/Compute/05_Extract_NOAA_Rep"
######################################################################################################################################################################
...
for Year in range(YearS,YearF+1):
      ...
      ff = df[df["EVENT_TYPE"].isin(["Flash Flood", "Flood", "Coastal Flood", "Heavy Rain", "Hurricane/Typhoon", "Tornado", "Tropical Storm"])]
      ...
      if num_rows_ff != 0:
            ...
            reduced_ff = ff[["EVENT_ID", "STATE", "CZ_TIMEZONE", "SOURCE", "FLOOD_CAUSE", "BEGIN_DATE_TIME", "END_DATE_TIME", "BEGIN_DATE_TIME_UTC", "END_DATE_TIME_UTC", "BEGIN_LAT", "BEGIN_LON", "END_LAT", "END_LON", "AREA_AFFECTED_CENTRE_LAT", "AREA_AFFECTED_CENTRE_LON", "AREA_AFFECTED_LENGTH", "AREA_AFFECTED_WIDTH"]]
