...
###########################################################################
# CODE DESCRIPTION
# 06_Compute_Mean_Std_AccTP_Analysis_ERA5_ecPoint.py Computes the   mean and the 
# standard deviation of rainfall analysis from ERA5-ecPoint over the considered accumulation 
# period.
...
DirOUT = "Data/Plot/06_Mean_Std_AccTP_Analysis_ERA5_ecPoint"
###########################################################################
...
Date_S = datetime(Year,9,1,12)
Date_F = datetime(Year,9,1,12)
...
while TheDate <= Date_F:
      ...
      for EndPeriod in range(0+Acc, 24+1, Acc):
            ...
            tp_bitmap = mv.values(mv.bitmap(tp, mask_bitmap))
            
            tp_perc90 = np.nanpercentile(tp_bitmap, 90, axis=0)
            print((tp_perc90))
            tp_perc99 = np.nanpercentile(tp_bitmap, 99, axis=0)
            print(((tp_perc99)))
            tp_stat = tp_perc99 - tp_perc90
            print((tp_stat))

            tp_perc_90 = np.nanpercentile(tp_bitmap, 90, axis=0)
            a = np.where(tp_perc_90 <= 1)[0]
            print(a.shape)
            ...
            title_plot1 = "ERA5-ecPoint " + str(Acc) + "-hourly tp (mm/" + str(Acc) + "h) - Mean"
            ...
            # mv.setoutput(png)
            mv.plot(geo_view, tp_mean_bitmap, contouring, legend, title)
