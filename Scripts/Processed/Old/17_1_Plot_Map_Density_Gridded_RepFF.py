...
# INPUT PARAMETERS
DateTime_Final_S = datetime(2005,1,1,12)
...
# Printing the number of flash flood report in the east/west and north/south of the USA
lats = mv.latitudes(Grid_RepFF_Total)
lons = mv.longitudes(Grid_RepFF_Total) - 360
lats_n = np.where(lats > 38)[0]
lats_s = np.where(lats <= 38)[0]
lons_w = np.where(lons <= -100)[0]
lons_e = np.where(lons > -100)[0]

Grid_RepFF_Total_vec = mv.values(Grid_RepFF_Total)
Grid_RepFF_Total_n = np.nansum(Grid_RepFF_Total_vec[lats_n])
Grid_RepFF_Total_s = np.nansum(Grid_RepFF_Total_vec[lats_s])
Grid_RepFF_Total_w = np.nansum(Grid_RepFF_Total_vec[lons_w])
Grid_RepFF_Total_e = np.nansum(Grid_RepFF_Total_vec[lons_e])
print(Grid_RepFF_Total_n, Grid_RepFF_Total_s, Grid_RepFF_Total_w, Grid_RepFF_Total_e)
