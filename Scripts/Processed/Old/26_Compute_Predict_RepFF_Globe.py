...
####################################################################################
# CODE DESCRIPTION
# 26_Compute_Predict_RepFF_Globe.py predicts the probabilities of having a flash flood event in a  
# given grid-box over the whole domain. 
# Runtime: the script can take up to 5 hours to compute in serial.
...
DirOUT = "Data/Compute/26_Predict_RepFF_Globe"
####################################################################################
...
FileIN = Git_Repo + "/" + DirIN_ML + "/" + Specification_PDT + "_" + Years_Train + "/weights_2HL_4N"
...
# Retrieval of the domain's mask
mask = mv.retrieve(
    class_ = "ea",
    date = "1940-01-01",
    expver = 1,
...
mask = mv.bitmap(mask, 0)
mask = (mask > 0) * 1 # selecting only land points
...
while TheDate <= Date_F:
      ...
      for EndPeriod in range(0+Acc, 24+1, Acc): 
            ...
            print()
            print("Creating prediction and computing shap values for: " + TheDateTime.strftime("%Y%m%d%H"))

            # Reading the percentage of soil moisture for the previous day, and extracting the nearest values to the mask grid-points 
            ...
            # Creating the predictions, and converting the in percentages from 0 to 100 %
