...
####################################################################################
# CODE DESCRIPTION
# 21_Compute_Predict_RepFF.py predicts the probabilities of having a flash flood report in a given 
# grid-box. 
...
model = tf.keras.Sequential([
    tf.keras.layers.InputLayer(input_shape=(5,)),  # Input layer specifying the input shape
...
while TheDate <= Date_F:
      ...
      for EndPeriod in range(0+Acc, 24+1, Acc): 
            ...
            predictors = np.concatenate((tp_cat_mask, ss_mask, slor_mask, stdor_mask, pd_mask), axis=1)
