...
Architecture = "3HL_16N"
Git_Repo = "/ec/vol/ecpoint_dev/mofp/Papers_2_Write/Use_FlashFloodsRep_4Verif_USA"
DirIN = "Data/Compute/19_Combine_PDT"
DirOUT = "Data/Compute/20_TrainML"
##########################################################################

# Define a callback to estimate the time taken to train the model at each epoch
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(16, activation=tf.nn.relu),  # First hidden dense layer with ReLU activation
    tf.keras.layers.Dense(16, activation=tf.nn.relu),  # Second hidden dense layer with ReLU activation
    tf.keras.layers.Dense(16, activation=tf.nn.relu),  # Third hidden dense layer with ReLU activation
...
# Extracting meta-information about the training
...
model.save_weights(
    MainDirOUT + "/weights_" + Architecture, overwrite=True, save_format=None, options=None
)
np.save(MainDirOUT + "/train_accuracy_" + Architecture, train_accuracy)
np.save(MainDirOUT + "/validation_accuracy_" + Architecture, validation_accuracy)
np.save(MainDirOUT + "/train_time_" + Architecture, train_time)