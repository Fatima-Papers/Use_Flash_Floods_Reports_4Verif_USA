...
##################################################################################################
# CODE DESCRIPTION
# 21_Compute_Predict_RepFF.py predicts the probabilities of having a flash flood event in a given grid-box. 
# Runtime: the script can take up to 3 hours to compute in serial.
...
# Creating the predictions for different reductions in the training dataset
for Perc_RedFF in Perc_RedFF_list:

      print()
      print("Considering the reduced training dataset of " + str(int(Perc_RedFF*100)) + "%") 
      ...
      FileIN = Git_Repo + "/" + DirIN_ML + "/" + Specification_PDT + "_" + Years_Train + "/weights_" + str(int(Perc_RedFF*100))
      ...
      while TheDate <= Date_F:
            ...
            for EndPeriod in range(0+Acc, 24+1, Acc): 
                  ...
                  MainDirOUT = Git_Repo + "/" + DirOUT + "/" + Specification_PDT + "_" + Years_Train + "/" + str(int(Perc_RedFF*100))
