...
##################################################################################################
# CODE DESCRIPTION
# 20_Compute_TrainML.py trains the ML model to predict the probabilities of having a flash flood event in a given 
# grid-box. 
...
# Reading the point data table for different reduced number of flash flood reports
for Perc_RedFF in Perc_RedFF_list:

    print()
    print("Training the ML model with reduced training dataset of: " + str(int(Perc_RedFF*100)) + "%")

    # Reading the training dataset
    FileIN_training = Git_Repo + "/" + DirIN +  "/pdt_" + Specification_PDT + "_" + str(int(Perc_RedFF*100)) + "_" + Years_Train + ".npy"
    ...
    model.save_weights(
        MainDirOUT + "/weights_" + str(int(Perc_RedFF*100)), overwrite=True, save_format=None, options=None
    )