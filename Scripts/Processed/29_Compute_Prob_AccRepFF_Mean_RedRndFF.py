...
# Runtime: the script can take up to 5 minutes to compute in serial.
...
Dir_IN_OUT = "Data/Compute/28_Prob_AccRepFF/RedRndFF_2005_2020"
#######################################################################################################
...
for NamePDT in NamePDT_list:
      ...
      for Perc_Red in Perc_Red_list:
            ...
            print("Computing the mean probabilities of the following randomly reduced training dataset: " + Dir_IN_OUT.split("/")[-1] + "_" + NamePDT + "_" + str(Perc_Red))
            ...
            while TheDateTime_Start <= TheDateTime_Start_F:
                  ...
                  for ind_Rep in range(Rep):
                        ...
                        FileIN = Git_Repo + "/" + Dir_IN_OUT  + "/" + NamePDT   + "/" + f"{Perc_Red:02}" + "/" + ind_Rep_STR + "/Prob_AccRepFF_"  +  TheDateTime_Final.strftime("%Y%m%d") + "_" + TheDateTime_Final.strftime("%H") + ".grib"
                  ...
                  MainDirOUT = Git_Repo + "/" + Dir_IN_OUT  + "/" + NamePDT   + "/" + f"{Perc_Red:02}" + "/Mean"
